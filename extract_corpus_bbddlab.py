...
# Variables
in_path = '/home/vocalyre/ws/empatia/interspeech_aasc_tfidf/yamnet_outputs/uc3m4safety_stimulus/binscores/'
out_path = '/home/vocalyre/ws/empatia/interspeech_aasc_tfidf/corpus/uc3m4safety_stimulus/'
...
def main():
    ...
    with open('/home/vocalyre/ws/empatia/tfidf_fear/ontology_noblacklist.json', 'r') as f:
    ...
    for file_name in sorted(glob.glob(os.path.join(in_path, '*_binary_logscores_th0027.csv'))):  # BBDD_LAB ALL
        n_file += 1
        input_df = pd.read_csv(file_name)
        file_name = file_name.replace(in_path, '')
        print(file_name)
        ...
        #     # label_str = id_df.loc[id_df['display_name'] == str(label_str)]['display_name']
        #     mids_str.append('/' + str(mid_str) + ',')
        #     # print(str(mids_str))
        #     # labels_str = labels_str.join(label_str) + ', '
        #     labels_str.append(str(label_str) + ',')
        #     # print(labels_str)
        #
        # mids_strstr = ' '.join(mids_str)  # list to string
        # labels_strstr = ' '.join(labels_str)
        #
        # # labels_corpus.append(labels_list)
        # mids_list = [mids_strstr, str(n_file)]
        # labels_list = [labels_strstr, str(n_file)]
        # mids_corpus.append(mids_list)
        # labels_corpus.append(labels_list)
        #
        # #####
        #
        # # Filter all zero, inf or nan columns
        # log_scores = log_scores.replace([np.inf, -np.inf, np.nan], 0)
        #
        # # Filter any column that does not present a single value above the threshold
        # filtered_log_scores = log_scores.loc[:, (log_scores >= THR).any(axis=0)]
        # print(filtered_log_scores.head())
    print('After filtering shape:' + str(np.shape(filtered_input_df)))
    # filtered_input_df.to_csv(path_or_buf=in_path + 'bbddlabstimulus_filtered_220325_logscores.csv', index=False)
    print('Saved\n-----\n')
    # print(np.shape(labels_corpus))
    # print(labels_corpus)
    # Save corpus
    # file = open(out_path + "corpus_mids_220401_bbddlab_allemotions_all_patchhop_1.txt", "w")  # BBDD_LAB (ALL)
    # file.write(str(mids_corpus))
    # file.close()

    # with open(out_path + "corpus_mids_220401_bbddlab_allemotions_all_patchhop_1.csv", "w") as f:  # BBDD_LAB (ALL)
    #     wr = csv.writer(f)
