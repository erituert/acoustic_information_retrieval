...
# Variables
in_path = './yamnet_outputs/uc3m4safety_stimulus/binscores/'
out_path = './corpus/uc3m4safety_stimulus/'
...
def main():
    ...
    filtered_input_df.to_csv(path_or_buf=in_path + 'bbddlabstimulus_filtered_220421_logscores.csv', index=False)
    print('Saved\n-----\n')

    # Save corpus
    file = open(out_path + "corpus_mids_220421_bbddlab_allemotions_all_patchhop_1.txt", "w")  # BBDD_LAB (ALL)
    file.write(str(mids_corpus))
    file.close()

    with open(out_path + "corpus_mids_220421_bbddlab_allemotions_all_patchhop_1.csv", "w") as f:  # BBDD_LAB (ALL)
        wr = csv.writer(f)
